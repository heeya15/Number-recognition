...
namespace Number_recognition
{
    class Start
    {
        ...
        // 그것을 막기위해서 [ 그래서 검은색 영역만 관심영역을 추출해서 처리 ]한다.  --> 그래서 ROI 를 하는거다. 
        ...
        public double[] membership(Bitmap bitmap) //FCM 부분  --클러스터링 레이어라고 보면된다. 
        {
            ...
            // 제일 왼쪽위 (0,0)
            // grayarray.GetLength(0) = 제일최대치 X축의  오른쪽제일위 
            // 왼쪽 제일 밑 
            // 오른쪽 제일밑에 
        ...
        public void num_FMMNN(Bitmap bitmap) // Fuzzy max-min 신경망 구조 
        {
            ...
            Console.WriteLine("인식한 숫자는 :" + per.Fuzzy_MMNN(input)); // 해당 인지한 4개의 값을 넘겨서 분류한다.
        }
